...
# Replace newlines temporarily to make regex easier across tags if needed.
# But string replaces are simpler.
...
# Delete items 03 and 04
text = re.sub(r'<div style="display:flex;align-items:flex-start;gap:12px;padding:12px 0;border-bottom:1px solid rgba\(0,0,0,0\.08\);">\s*<div style="font-family: \'Newsreader\', serif; font-style: italic;font-size:20px;color:var\(--green\);min-width:32px;line-height:1\.1;">03</div>.*?March 22nd\. The scar is fresh\.</div>\s*</div>\s*</div>', '', text, flags=re.DOTALL)


# ==========================================
# SLIDE 3 (Mapped to old slide 3)
# ==========================================
...
# Delete Saka and Gyokeres blocks
text = re.sub(r'<div style="display:flex;align-items:flex-start;gap:14px;margin-bottom:16px;">\s*<div style="font-size:22px;line-height:1;">⚡</div>.*?Gyökeres doing Gyökeres things\.</div>\s*</div>\s*</div>', '', text, flags=re.DOTALL)


# ==========================================
# SLIDE 4 (Mapped to old slide 4 W/D/L)
# ==========================================
...
# Delete L block
text = re.sub(r'<div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:12px;">\s*<div style="font-family: \'Newsreader\', serif; font-style: italic;font-size:20px;color:#DC2626;min-width:32px;line-height:1\.1;">L</div>.*?City still breathing\. Every remaining game a cup final\.</div>\s*</div>', '', text, flags=re.DOTALL)
...
# ==========================================
# SLIDE 6 (Mapped to old slide 6 Fixtures)
# ==========================================
text = text.replace('>The Run-In<', '>THE GAUNTLET<')
text = text.replace('FIVE GAMES.<br/>ONE DREAM. 🏆', 'THE FIXTURES<br/>THAT DEFINE US ⚔️')

text = text.replace('>APR 15<', '>APR 27<')
text = text.replace('vs Sporting CP (UCL QF 2nd leg)', '@ Spurs (The NLD)')
text = text.replace('>H<', '>A<')  # Wait, there are multiple H and A.
# Let's do exact replaces for Fixture 1
...
# Delete extra fixtures (MAY 3, MAY 24)
text = re.sub(r'<div style="display:flex;align-items:center;gap:12px;padding:12px 14px;border-bottom:1px solid rgba\(255,255,255,0\.08\);">\s*<div style="font-family: \'Newsreader\', serif; font-style: italic;font-size:28px;color:#fff;line-height:1;">MAY 3</div>.*?</div>\s*</div>\s*</div>', '', text, flags=re.DOTALL)


# ==========================================
# SLIDE 7 (Mapped to old slide 7 CTA)
# ==========================================
...
# Finally, write it to arsenal_carousel 2.html
